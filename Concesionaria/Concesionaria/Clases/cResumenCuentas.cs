...
namespace Concesionaria.Clases
{
    public class cResumenCuentas
    {
        ...
        public DataTable GetResumenCuentas(int CodMoneda)
        {
            //obtengos los importes sin procesar
            string sql = "select sum(isnull(ImporteEfectivo,0)) as ImporteEfectivo, sum(isnull(ImportePrenda,0)) as ImportePrenda";
            sql = sql + ", sum(isnull(ImporteAuto,0)) as ImporteAuto, sum(isnull(ImporteCobranza,0)) as ImporteCobranza, sum(ImporteBanco) as ImporteBanco";
            sql = sql + " from Movimiento";
            ...
            //obtengo los resumenes
            /*
            string sql2 = "select * from ResumenCuentas where CodMoneda=" + CodMoneda.ToString();
            DataTable tResumen = cDb.ExecuteDataTable(sql2);
            double ImporteEfectivo = 0;
            double ImportePrenda = 0;
            double ImporteAuto = 0;
            ...
            //actualizo los resumenes nuevos
            string sql3 = "delete from ResumenCuentas where CodMoneda=" + CodMoneda.ToString();
            cDb.ExecutarNonQuery(sql3);

            string sql4 = "Insert into ResumenCuentas(ImporteEfectivo,ImportePrenda,ImporteAuto,ImporteCobranza,ImporteBanco,CodMoneda)";
            sql4 = sql4 + " values (" + ImporteEfectivo.ToString().Replace(",", ".");
            sql4 = sql4 + "," + ImportePrenda.ToString().Replace(",", ".");
            sql4 = sql4 + "," + ImporteAuto.ToString().Replace(",", ".");
            sql4 = sql4 + "," + ImporteCobranza.ToString().Replace(",", ".");
            sql4 = sql4 + "," + ImporteBanco.ToString().Replace(",", ".");
            sql4 = sql4 + "," + CodMoneda.ToString();
            sql4 = sql4 + ")";
            cDb.ExecutarNonQuery(sql4);
            //actualizo los movimientos
            string sql5 = "Update Movimiento set Procesado =1";
        ...
        public Double GetEfectivo(int CodMoneda)
        {
            Double Importe = 0;
            string sql = "select sum(ImporteEfectivo) as Importe ";
            sql = sql + " from Movimiento ";
            sql = sql + " where CodMoneda = " + CodMoneda.ToString();
